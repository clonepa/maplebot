...
def export_collection_to_list(user):
    ...
    conn = sqlite3.connect('maple.db')
    c = conn.cursor()
    c.execute("SELECT SUM(amount_owned), card_name, card_set, card_type, rarity FROM collection INNER JOIN cards ON collection.multiverse_id = cards.multiverse_id WHERE owner_id = :ownerid GROUP BY card_name ORDER BY SUM(amount_owned) DESC", {"ownerid": who[0]})
    out = []
